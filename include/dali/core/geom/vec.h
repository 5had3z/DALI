...
template <typename Arg1, typename Arg2, bool is_fp1 = std::is_floating_point<Arg1>::value,
...
template <typename VecElement, typename Scalar,
...
struct vec_base {
  ...
  template <typename... Components, typename = std::enable_if_t<sizeof...(Components) == N>>
  DALI_HOST_DEV constexpr vec_base(Components... components) : v{T(components)...} {}  // NOLINT
  T v[N];
};
...
struct vec : vec_base<N, T> {
  ...
  template <typename... Components, typename = std::enable_if_t<sizeof...(Components) == N>>
  DALI_HOST_DEV constexpr vec(Components... components)
      : vec_base<N, T>(components...) {}  // NOLINT

  using vec_base<N, T>::v;

  template <typename U>
  DALI_HOST_DEV explicit constexpr vec(const mat<N, 1, U> &m) : vec(m.col(0).template cast<T>()) {}

  DALI_HOST_DEV
  constexpr vec(const mat<N, 1, T> &m) : vec(m.col(0).template cast<T>()) {}  // NOLINT

  template <typename U>
  DALI_HOST_DEV explicit constexpr vec(const vec<N, U> &v) : vec(v.template cast<T>()) {}

  DALI_HOST_DEV DALI_FORCEINLINE constexpr T &operator[](int i) {
    return v[i];
  }
  DALI_HOST_DEV DALI_FORCEINLINE constexpr const T &operator[](int i) const {
    return v[i];
  }

  template <typename U>
  DALI_HOST_DEV DALI_FORCEINLINE constexpr vec<N, U> cast() const {
    vec<N, U> ret = {};
    for (int i = 0; i < N; i++) {
  ...
  DALI_HOST_DEV DALI_FORCEINLINE constexpr int size() const {
    return N;
  }

  DALI_HOST_DEV constexpr T *begin() {
    return &v[0];
  }
  DALI_HOST_DEV constexpr const T *cbegin() const {
    return &v[0];
  }
  DALI_HOST_DEV constexpr const T *begin() const {
    return &v[0];
  }
  DALI_HOST_DEV constexpr T *end() {
    return &v[N];
  }
  DALI_HOST_DEV constexpr const T *cend() const {
    return &v[N];
  }
  DALI_HOST_DEV constexpr const T *end() const {
    return &v[N];
  }
  ...
  DALI_HOST_DEV DALI_FORCEINLINE constexpr vec operator+() const {
    return *this;
  }
...
#define DEFINE_ASSIGN_VEC_OP(op)                                                           \
  ...
  template <typename U>                                                                    \
  DALI_HOST_DEV DALI_FORCEINLINE std::enable_if_t<is_scalar<U>::value, vec &> operator op( \
      const U &rhs) {                                                                      \
    for (int i = 0; i < N; i++)                                                            \
      v[i] op rhs;                                                                         \
    return *this;                                                                          \
...
DALI_HOST_DEV DALI_FORCEINLINE constexpr auto dot(const vec<N, T> &a, const vec<N, U> &b) {
...
template <typename T, typename U>
DALI_HOST_DEV DALI_FORCEINLINE constexpr auto cross(const vec<3, T> &a, const vec<3, U> &b) {
  using R = decltype(a[0] * b[0] + a[0] * b[0]);
  return vec<3, R>{a.y * b.z - b.y * a.z, a.z * b.x - b.z * a.x, a.x * b.y - b.x * a.y};
}

/// @brief Calculates `z` coordinate of a cross product of two 2D vectors
template <typename T, typename U>
DALI_HOST_DEV DALI_FORCEINLINE constexpr auto cross(const vec<2, T> &a, const vec<2, U> &b) {
  return a.x * b.y - b.x * a.y;
}

#define DEFINE_ELEMENTIWSE_VEC_BIN_OP(op)                                                   \
  template <int N, typename T, typename U>                                                  \
  DALI_HOST_DEV DALI_FORCEINLINE constexpr auto operator op(const vec<N, T> &a,             \
                                                            const vec<N, U> &b) {           \
  ...
  DALI_HOST_DEV DALI_FORCEINLINE constexpr std::enable_if_t<is_scalar<U>::value, vec<N, R>> \
  ...
  DALI_HOST_DEV DALI_FORCEINLINE constexpr std::enable_if_t<is_scalar<T>::value, vec<N, R>> \
...
#define DEFINE_SHIFT_VEC_BIN_OP(op)                                                         \
  template <int N, typename T, typename U>                                                  \
  DALI_HOST_DEV DALI_FORCEINLINE constexpr vec<N, T> operator op(const vec<N, T> &a,        \
                                                                 const vec<N, U> &b) {      \
    vec<N, T> ret{};                                                                        \
    for (int i = 0; i < N; i++)                                                             \
      ret[i] = a[i] op b[i];                                                                \
  ...
  template <int N, typename T, typename U>                                                  \
  DALI_HOST_DEV DALI_FORCEINLINE constexpr std::enable_if_t<is_scalar<U>::value, vec<N, T>> \
  operator op(const vec<N, T> &a, const U &b) {                                             \
    vec<N, T> ret{};                                                                        \
    for (int i = 0; i < N; i++)                                                             \
      ret[i] = a[i] op b;                                                                   \
  ...
  template <int N, typename T, typename U>                                                  \
  DALI_HOST_DEV DALI_FORCEINLINE constexpr std::enable_if_t<is_scalar<T>::value, vec<N, T>> \
  operator op(const T &a, const vec<N, U> &b) {                                             \
    vec<N, T> ret{};                                                                        \
    for (int i = 0; i < N; i++)                                                             \
      ret[i] = a op b[i];                                                                   \
...
template <int N, typename T>
DALI_HOST_DEV constexpr vec<N, T> clamp(const vec<N, T> &in, const vec<N, T> &lo,
                                        const vec<N, T> &hi) {
...
template <int N, typename T, typename U>
DALI_HOST_DEV constexpr std::enable_if_t<std::is_integral<T>::value && std::is_integral<U>::value,
...
DALI_HOST_DEV DALI_FORCEINLINE constexpr auto cat(const vec<size0, T> &v0,
                                                  const vec<size1, T> &v1) {
...
DALI_HOST_DEV DALI_FORCEINLINE constexpr auto cat(const vec<size0, T> &v0, T v1) {
  vec<size0 + 1, T> ret = {};
  for (int i = 0; i < size0; i++) {
    ret[i] = v0[i];
  }
...
DALI_HOST_DEV DALI_FORCEINLINE constexpr auto cat(T v0, const vec<size1, T> &v1) {
  vec<size1 + 1, T> ret = {};
  ret[0] = v0;
  for (int i = 0; i < size1; i++) {
    ret[i + 1] = v1[i];
...
DALI_HOST_DEV DALI_FORCEINLINE constexpr auto cat(const vec<size0, T> &v0,
                                                  const vec<sizes, T> &...tail) {
  return cat(v0, cat(tail...));
}

template <int sub_n, int n, typename T>
DALI_HOST_DEV DALI_FORCEINLINE constexpr auto sub(const vec<n, T> &orig, int start = 0) {
...
DALI_HOST_DEV DALI_FORCEINLINE constexpr vec<sizeof...(indices), T> shuffle(const vec<N, T> &v) {
  static_assert(all_of<(indices < N)...>::value, "Vector component index out of range");
  return {v[indices]...};
}

static_assert(std::is_trivial<vec<1>>::value && std::is_standard_layout<vec<1>>::value,
              "vec<1, T> must be a POD type");
static_assert(std::is_trivial<vec<2>>::value && std::is_standard_layout<vec<2>>::value,
              "vec<2, T> must be a POD type");
static_assert(std::is_trivial<vec<3>>::value && std::is_standard_layout<vec<3>>::value,
              "vec<3, T> must be a POD type");
static_assert(std::is_trivial<vec<4>>::value && std::is_standard_layout<vec<4>>::value,
              "vec<4, T> must be a POD type");
static_assert(std::is_trivial<vec<5>>::value && std::is_standard_layout<vec<5>>::value,
              "vec<N, T> must be a POD type");
